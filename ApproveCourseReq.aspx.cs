...
{
    public partial class ApproveCourseReq : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            bool flag2 = (bool)checkifcourse(id);
            if (flag2)
            {
                if (flag)
                {
                    string code = sem_code.Text;
                    SqlCommand approve = new SqlCommand("Procedures_AdvisorApproveRejectCourseRequest", conn);
                    approve.CommandType = CommandType.StoredProcedure;
                    approve.Parameters.Add("@requestID", id);
                    approve.Parameters.Add("@current_semester_code", code);
        ...
        private bool checkifcourse(int req_id)
        {
            string connStr = WebConfigurationManager.ConnectionStrings["GUC_SYSTEM"].ConnectionString;
            SqlConnection conn = new SqlConnection(connStr);
            SqlCommand search = new SqlCommand("SELECT Count(*) FROM Request WHERE type LIKE '%Course%' AND request_id = " + req_id, conn);
